            ...
            if len(longest_AccessionID_list) != 0:
                if accession in longest_AccessionID_list:
                    ...
                    chain_total_renum.append(
                        [mmCIF_name[:4] + "*", chain, chain_PDBe_PDB[chain], accession, AccessionID_human_read_longest,
                         prot_len, count_accession_len, total_count_per_chain, renum_for_accession, renum5000])
                else:
                    AccessionID_human_read = UniProt_conversion_dict.get(accession)
                    chain_total_renum.append(
                        [mmCIF_name[:4], chain, chain_PDBe_PDB[chain], accession, AccessionID_human_read,
                         prot_len, count_accession_len, total_count_per_chain, renum_for_accession, renum5000])
            else:
                AccessionID_human_read = UniProt_conversion_dict.get(accession)
                chain_total_renum.append(
